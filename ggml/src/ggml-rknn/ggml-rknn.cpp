...
struct ggml_rknpu_matmul_pair create_matmul_pair(int M, int K, int N, rknn_matmul_type type, int thread_idx, char * name, int num_cores = local_rknn_config["num_cores"].get<int>()) {
    ...
    if (part_AC == NULL) {
        ...
        if (part_AC->prefill) {
            ...
            info.type = type;
            info.B_layout = 1;
            info.AC_layout = 0; // Use normal layout (RKNN_MM_LAYOUT_NORM) for faster prefill
...
/**
 * @brief Convert F32 to F16 in normal 2D layout (for AC_layout=NORM)
 * src: GGML column-major (K, M) -> accessed as src[k + m * K]
 * dst: RKNN row-major (M, K) -> accessed as dst[m * K + k]
 * 
 * Since GGML stores intermediate values in column-major order,
 * and RKNN expects row-major order, we need to transpose during conversion.
 */
void convert_ggml_to_rknn_norm_layout_A_f16(const float * src, rknpu2::float16 * dst, int32_t M, int32_t K) {
    #pragma omp parallel for num_threads(omp_threads) schedule(static)
    for (int m = 0; m < M; m++) {
...
/**
 * @brief Quantize F32 to INT8 in normal 2D layout (for AC_layout=NORM) 
 * Quantizes per row and stores scale factors in delta array
 */
void convert_ggml_to_rknn_norm_layout_A_q8(const float * src, int8_t * dst, float * delta, int32_t M, int32_t K) {
    #pragma omp parallel for num_threads(omp_threads) schedule(static)
    for (int m = 0; m < M; m++) {
        const float* src_row = src + m * K;
...
/**
 * @brief Copy C result from RKNN norm layout to GGML layout (F32)
 * src: RKNN row-major (M, N) - result from matmul
 * dst: GGML column-major (N, M) -> accessed as dst[n + m * ori_N]
 * 
 * For a sub-matrix operation with col_start offset
 */
void copy_rknn_norm_layout_C_to_ggml_f32(const float * src, float * dst, int32_t M, int32_t N, int32_t ori_N, int32_t col_start) {
    #pragma omp parallel for num_threads(omp_threads) schedule(static)
    for (int m = 0; m < M; m++) {
        const float* src_row = src + m * N;
...
/**
 * @brief Dequantize INT32 result from RKNN to F32 in GGML layout
 * Applies per-row scale from A and per-column scale from B
 */
void copy_rknn_norm_layout_C_to_ggml_q8(const int32_t * src, float * dst, const float * delta_a, const float * delta_b, int32_t M, int32_t N, int32_t ori_N, int32_t col_start) {
    #pragma omp parallel for num_threads(omp_threads) schedule(static)
    for (int m = 0; m < M; m++) {
        const int32_t* src_row = src + m * N;
        float* dst_row = dst + m * ori_N + col_start;
                        ...
                        int num_cores = 3)
{
    ...
    {
        // Now C uses norm layout (2D: M x N) since AC_layout=0

        if (type == RKNN_FLOAT16_MM_FLOAT16_TO_FLOAT32){
            TIMEIT(
                // Use norm layout copy (2D layout: M x N) instead of perf layout
                copy_rknn_norm_layout_C_to_ggml_f32((float *)part_AC->C->virt_addr, (float *)dst->data, M, N, ori_N, col_start);
            ...
            TIMEIT(
                // Use norm layout dequantize (2D layout: M x N) instead of perf layout
                copy_rknn_norm_layout_C_to_ggml_q8((int32_t *)part_AC->C->virt_addr, (float *)dst->data, (float *)A_delta, (float *)B_data_delta, M, N, ori_N, col_start);
                part_AC->is_using = false;
            , &durations[8]);
        }
    }
